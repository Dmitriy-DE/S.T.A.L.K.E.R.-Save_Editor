...
**Сейчас:** импортированная v0.3.0 EXPERIMENTAL, Python + Tkinter, bundled decoder для Linux x86_64. Это исследовательская версия, не готовый универсальный редактор. Windows-приложение и новый Qt-интерфейс запланированы, но ещё не реализованы.
...
Cloud-процесс в исходниках содержит fresh SHA, backup, persisted и read-back. Его работа с реальным Steam/GFN в этом репозитории ещё не проверена. В CLI нет обязательного backup, локальная запись не атомарна: до S03 сохраняйте копию вручную и пишите в новый файл. Остальные ограничения: [состояние и пробелы](docs/STATUS.md).
