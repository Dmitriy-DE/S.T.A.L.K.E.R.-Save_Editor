...
Онлайн-версия: содержимое `web/` выложено в ветку `gh-pages`
(`make web-publish`). Чтобы она открылась по ссылке, Pages включается один
раз в Settings → Pages → Deploy from a branch → `gh-pages` → `/ (root)`.
