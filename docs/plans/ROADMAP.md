...
- Новые возможности не считаются сделанными из-за наличия этого плана. Версия поднимается по фактически принятому результату; текущая — 0.4.3 (импортированный baseline был 0.3.0-experimental).
...
| 9 — полный редактор | M10→M11, затем M12/M13/M14→M15 и M16 | Ремонт, предметы, отношения с группировками, принадлежность игрока, запись в папку игры с откатом. Каждая запись — после проверки загрузкой в игре. [План](FULL_EDITOR.md) |
