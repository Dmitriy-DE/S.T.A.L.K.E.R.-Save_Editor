...
**Architecture:** общее Python-ядро, immutable edit requests и единая безопасная запись для CLI/Tk/Qt. Новый UI на PySide6, нативные decoder/helper для каждой ОС. Новые поля save добавляются через evidence gates.

**Tech Stack:** Python 3.11/3.12, pyooz 0.0.8, PySide6, pytest/pytest-qt, PyInstaller, GitHub Actions.
...
| 4 — выпуск | B01→B02 | Две standalone сборки, checksums, local acceptance; cloud claim отдельно по evidence |
