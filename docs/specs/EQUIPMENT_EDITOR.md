...
weapons, armor and helmets as user-facing categories, while preserving the
format serializer family as a separate technical field. An item can be in the
equipped set or the backpack/inventory set; an unknown placement is shown as
...
filters, text search, and sorting by name, durability, or category. A row shows
icon when an exact catalog definition supplies one, name, category, location,
current durability, upgrades, and a read-only reason when the field is not
safe to edit. Technical fields remain available in a details tooltip.
...
must all pass the current bounds and ambiguity guards. Call of Pripyat helmets
are displayed as `helmet` even when their serializer family is `outfit`.
