...
Preview: путь назначения, локально/cloud, поля before→after, рассчитанный размер, backup path, experimental marker. `FormatCapabilities.experimental_fields` перечисляет source-backed правки, которые UI обязан явно пометить как experimental; desktop перед записью требует backup, а browser предупреждает о ручной копии и скачивает новый файл без перезаписи источника. Progress: чтение → backup → проверка → запись → подтверждение; ошибки понятным текстом с раскрываемыми technical details и кнопкой копирования отчёта без credentials/save bytes. Cloud tab требует явного connect/list/analyze; показывает только `Data/*.sav`, закрепляет выбранный remote locator и отображает `verified`/`uncertain` receipt. После `WriteFile` повторная запись из того же preview запрещена.
