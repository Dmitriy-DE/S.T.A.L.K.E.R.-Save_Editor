# X-Ray relations and player community — 2026-09-15
...
`community` игрока — отдельный signed `s32` в actor STATE после money,
specific-character, trader flags и profile. Его значение сверяется только с
numeric id из release-scoped `[game_relations]` каталога.
...
- `SaveInfo` теперь показывает `player_faction_index` и только реально
  сохранённые community goodwill rows.
...
- `EditPlan.faction_relations` и `EditPlan.player_faction` валидируют exact
  release-scoped keys; чужой или неизвестный numeric id отклоняется.
- Synthetic tests проверяют patch одной строки, добавление отсутствующей
  строки, сохранение personal/other rows, смену community и round-trip.
