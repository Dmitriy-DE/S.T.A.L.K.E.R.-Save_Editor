...
not specify the offline `.sav` serializer. The community
...
`editor/s2_catalog.py` reads only loose files below the canonical official
`Content/GameLite/GameData/ItemPrototypes` directory. It rejects obvious mod
roots, ignores symlinks and malformed files, and never scans PAK archives.
