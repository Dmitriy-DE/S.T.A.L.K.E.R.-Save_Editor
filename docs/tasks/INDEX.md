...
Текущий результат — **S05 / issue #5**. S01–S04 приняты в `main`; S05 выполняется на ветке `task/s05`. Все 28 issues созданы. `waiting_dependencies` означает, что входной gate ещё не принят, `in_progress` — ветка выполняется, `in_review` — PR открыт, `accepted` — PR слит и проверки записаны. Одна карточка — один PR.
...
| [S05](S05.md) | Steam worker: restart, timeout и единственный reader | S01 | in_progress | [#5](https://github.com/Dmitriy-DE/S.T.A.L.K.E.R.-2-HoC---Save_Editor/issues/5) |
