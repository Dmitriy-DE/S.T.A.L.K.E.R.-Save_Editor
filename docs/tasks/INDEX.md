...
Текущий результат — **U04 / issue #13**. S01–S06, P01 и P02, код P03, U01, U02 и U03 приняты в `main`; P03 runner gate остаётся открытым после четырёх `startup_failure` запусков без jobs. U04 добавляет immutable preview, stale-SHA guard и local apply через общий storage; backup/restore и Steam UI остаются U05/U06. Все 28 issues созданы. `waiting_dependencies` означает, что входной gate ещё не принят, `in_progress` — ветка выполняется, `in_review` — PR открыт, `accepted` — PR слит и проверки записаны. Одна карточка — один PR.
...
| [U04](U04.md) | Предпросмотр изменений и безопасный apply | U03 | in_progress | [#13](https://github.com/Dmitriy-DE/S.T.A.L.K.E.R.-2-HoC---Save_Editor/issues/13) |
