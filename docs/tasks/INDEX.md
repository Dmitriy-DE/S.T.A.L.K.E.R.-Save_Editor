...
Текущий результат — **U01 / issue #10**. S01–S06, P01 и P02, а также код P03 приняты в `main`; P03 runner gate остаётся открытым после двух `startup_failure` запусков без jobs. U01 — следующий независимый code gate для общего service слоя. Все 28 issues созданы. `waiting_dependencies` означает, что входной gate ещё не принят, `in_progress` — ветка выполняется, `in_review` — PR открыт, `accepted` — PR слит и проверки записаны. Одна карточка — один PR.
...
| [U01](U01.md) | Общий service слой для GUI и CLI | S02, S03, S04, S06, P02 | in_progress | [#10](https://github.com/Dmitriy-DE/S.T.A.L.K.E.R.-2-HoC---Save_Editor/issues/10) |
