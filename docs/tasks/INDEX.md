...
Текущий результат — **S03 / issue #3**. S01 и S02 приняты в `main`; S03 выполняется на ветке `task/s03`. Все 28 issues созданы. `waiting_dependencies` означает, что входной gate ещё не принят, `in_progress` — ветка выполняется, `in_review` — PR открыт, `accepted` — PR слит и проверки записаны. Одна карточка — один PR.
...
| [S03](S03.md) | Единая безопасная локальная запись для GUI и CLI | S02 | in_progress | [#3](https://github.com/Dmitriy-DE/S.T.A.L.K.E.R.-2-HoC---Save_Editor/issues/3) |
