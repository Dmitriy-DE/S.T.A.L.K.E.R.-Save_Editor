...
`ui/cloud_view.py` imports `discover_helper` from `steam_cloud`, which only
re-exports it from `editor.platforms`.  Nothing in the suite used to import that
module, so a broken re-export stayed green while the UI failed on startup.
These tests close that gap: they only import, they never build a window or
contact Steam.
"""
